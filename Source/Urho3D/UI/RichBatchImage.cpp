...
void RichWidgetImage::SetImageSource(const String& sourceUrl) {
  if (!texture_ || sourceUrl != texture_->GetName()) {
    if (context_->GetSubsystem<ResourceCache>()->Exists(sourceUrl))
      texture_ = (Texture*)context_->GetSubsystem<ResourceCache>()->GetResource<Texture2D>(sourceUrl);
...
void RichWidgetImage::AddImage(const Vector3 pos, float width, float height) {
  if (parent_widget_ && parent_widget_->GetShadowEnabled()) {
		AddQuad(FloatRect(
					pos.x_ + parent_widget_->GetShadowOffset().x_, 
				...
				pos.z_ + 0.01,
				FloatRect(0, 0, 0, 0), // Note that this UV is empty
				parent_widget_->GetShadowColor());
	}
	AddQuad(FloatRect(pos.x_, pos.y_, pos.x_ + width, pos.y_ + height), pos.z_, FloatRect(0.0f, 0.0f, 1.0f, 1.0f), Color::WHITE);
}
